...
path_of_groups = path.join(bundle_dir, 'groups.csv')
...
try:
    df_raw = read_csv(path_to_dat, encoding='big5')
except:
    df_raw = read_csv(path_to_dat)
...
def run_Girvan(df):
    ...
    for com in itertools.islice(communities, 100):
        p.append(list(com))
    k = []
...
run_Girvan(df_raw)
df_raw.to_csv(path_to_result,encoding = 'utf-8_sig',index = None)
